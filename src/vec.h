...
#define MAKE_VECTOR(TYPE, TNAME)                                               \
  ...
  TYPE vector_##TNAME##_pop(struct vector_##TNAME *vec) {                      \
    ...
    TYPE elem = vec->data[(vec->length - 1) * sizeof(TYPE)];                   \
    vec->length--;                                                             \
    return elem;                                                               \
  }                                                                            \
  size_t vector_##TNAME##_push(struct vector_##TNAME *vec, TYPE elem) {          \
    if (vec->length >= vec->cap) {                                             \
      size_t new_len = (vec->cap + (vec->cap >> 2)) * sizeof(TYPE);            \
      vec->data = realloc(vec->data, new_len);                                 \
      vec->cap = new_len;                                                      \
    }
    size_t inserted_idx = vec->length;                                                \
    vec->data[vec->length * sizeof(TYPE)] = elem;                              \
    vec->length++;                                                             \
    return inserted_idx; \
  }                                                                            \
  TYPE vector_##TNAME##_index(struct vector_##TNAME *vec, size_t idx) {        \
    ...
    return vec->data[idx * sizeof(TYPE)];                                      \
  }                                                                            \
  void vector_##TNAME##_set(struct vector_##TNAME *vec, TYPE elem, size_t idx) {        \
    if (idx < 0 || idx >= vec->length) {                                       \
      RUNTIME_ERROR("Indexing vector out of bounds");                          \
    }                                                                          \
    vec->data[idx * sizeof(TYPE)] = elem;                                      \
  }                                                                            \
